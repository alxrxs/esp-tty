...
/* Two-network boot-time state machine.
 *
 * Opt-in: only available when WIFI_ENTERPRISE_SSID is defined in config.h.
 * Reads the NVS credential store, decides whether bootstrap or enterprise
 * mode is appropriate, optionally syncs NTP and/or runs SCEP enrollment,
 * and blocks until the enterprise network has an IP or all paths are
 * exhausted.
 *
 ...
 *
 * Requires cred_store.h to be on the include path (it is part of
 * lib/cred_store/ and listed in CMakeLists INCLUDE_DIRS).
 *
