...
$EDITOR main/config.h     # at minimum: WIFI_SSID, WIFI_PASS,
                          # AUTHORIZED_PUBKEYS, OTA_AUTHORIZED_PUBKEY.
...
make ota alpha              # per-device config (see below)
```

`make ota` builds the current `main/config.h` (or the `config.<devname>.h`
you named) and runs `scripts/ota_send.py <host> .pio/build/esp32s3/firmware.bin`.
...
Each per-device config is `main/config.<devname>.h`. The first line is a
marker the Makefile reads to find the OTA target:
...
`make flash <devname>` symlinks `main/config.h -> config.<devname>.h`,
then flashes. `make ota <devname>` does the same and uploads to the
marker IP. `make ota <ip-or-host>` (any argument that doesn't match a
`config.<name>.h` file) builds the current `config.h` as-is and uploads
to that destination:

```
make flash alpha            # symlinks then flashes
make ota   alpha            # symlinks then OTAs to MAKE-OTA-IP
make ota   192.168.1.42     # builds current config.h, OTAs to IP
make build                  # uses current config.h unchanged
```

Because `main/config.h` is a symlink, in-place edits flow through to
the underlying `config.<devname>.h` file -- no separate "save" step.
Switching devices just repoints the symlink. `main/config.h` itself is
gitignored.
...
| [`lib/`](lib/README.md) | Platform-agnostic libraries: `ring`, `bridge`, `scrollback`, `pubkey_auth`, `cred_store`, `scep_proto`, `scep_transport`, `wifi_state`, `cert_renewer`, `rollback_decision`, `ssh_keepalive`, `term_resize`, `usb_cdc_drain`, `mdns_dispatch` |
